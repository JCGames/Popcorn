...
void AST::print_statement(Node* node, std::string indent)
{
    if (node == NULL)
    {
        printf("NULL\n");
    ...
    printf("%s%s\n", indent.c_str(), get_node_type_name(node->get_type()).c_str());
    switch (node->get_type())
    {
        ...
        case NodeType::NEGATE:
            print_statement(node->get_struct<UnaryOperator_S>()->child, indent + '\t');
            break;
        ...
        case NodeType::POWER_OPERATOR:
            printf("%sLEFT:\n", indent.c_str());
            print_statement(node->get_struct<BinaryOperator_S>()->left, indent + '\t');
            printf("%sRIGHT:\n", indent.c_str());
            print_statement(node->get_struct<BinaryOperator_S>()->right, indent + '\t');
            break;
        case NodeType::DOUBLE:
            printf("%s%f\n", indent.c_str(), *node->get_struct<double>());
            break;
        case NodeType::INTEGER:
            printf("%s%i\n", indent.c_str(), *node->get_struct<int>());
            break;
        case NodeType::STRING:
            printf("%s%s\n", indent.c_str(), (*node->get_struct<std::string>()).c_str());
            break;
        ...
        case NodeType::VARIABLE_ASSIGNMENT:
            ...
            break;
        case NodeType::BLOCK:
            for (auto statement : node->get_struct<Block_S>()->statements)
                print_statement(statement, indent + '\t');
            break;
        case NodeType::FUNCTION_CALL:
            printf("%sFunction Name: %s\n", indent.c_str(), node->get_struct<FunctionCall_S>()->functionName.c_str());
            printf("%s---PARAMETER LIST:\n", indent.c_str());
            for (auto e : node->get_struct<FunctionCall_S>()->paramValues)
                print_statement(e, indent + '\t');
            break;
        case NodeType::BOOLEAN:
            if (node->get_struct<bool>())
                printf("%sTRUE\n", indent.c_str());
            else
                printf("%sFALSE\n", indent.c_str());
            break;
        case NodeType::IF:
            printf("%sCONDITION:\n", indent.c_str());
            print_statement(node->get_struct<If_S>()->condition, indent + '\t');
            printf("%sBODY:\n", indent.c_str());
            print_statement(node->get_struct<If_S>()->body, indent + '\t');

            if (node->get_struct<If_S>()->branchingElseOrIf != nullptr)
            {
                printf("%sCHILD STATEMENT:\n", indent.c_str());
                print_statement(node->get_struct<If_S>()->branchingElseOrIf, indent + '\t');
            }
            break;
        case NodeType::ELSE:
            printf("%sBODY:\n", indent.c_str());
            print_statement(node->get_struct<Else_S>()->body, indent + '\t');
            break;
        case NodeType::WHILE:
            printf("%sCONDITION:\n", indent.c_str());
            print_statement(node->get_struct<While_S>()->condition, indent + '\t');
            printf("%sBODY:\n", indent.c_str());
            print_statement(node->get_struct<While_S>()->body, indent + '\t');
            break;
        case NodeType::FUNCTION:
            {
                auto func_struct = node->get_struct<Funciton_S>();
        ...
        case NodeType::RETURN:
            if (!node->is_struct_null())
                print_statement(node->get_struct<Return_S>()->expression, indent + '\t');
            break;
        case NodeType::MEMBER_ACCESSOR:
            printf("%sCLASS ACCESSED:\n", indent.c_str());
